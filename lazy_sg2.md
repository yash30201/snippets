# Lazy segment tree (Range addition, Range sum)

```cpp
struct lazy_seg_tree_range_add_range_sum_query{
	vector<int> t, lazy;
	int n, inf;
	lazy_seg_tree_range_add_range_sum_query(int N){
		n = N;
		inf = -1e9;
		t.assign(4*n, 0);
		lazy.assign(4*n, inf);
	...
	void addRange(int l, int r, int val, int ix, int lx, int rx){
		if(lx >= l && rx <= r){
		...
		if(rx - lx > 1) push(ix, lx, rx);
		int m = (lx + rx) >> 1;
		addRange(l, r, val, ix << 1, lx, m);
		addRange(l, r, val, ix << 1 | 1, m, rx);
		t[ix] = t[ix << 1] + t[ix << 1 | 1];
	}
	void addRange(int l, int r, int val){ // add in [l, r)
		addRange(l, r, val, 1, 0, n);
	}
